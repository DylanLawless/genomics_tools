...
dataLine <- subset %>%
  ...
  summarize(mean_x = mean(log(`Selective pressure`) ))
...
subset$Gene <- reorder(subset$Gene, subset$`Selective pressure`)

#subset %>% 
#  ggplot(aes(x=Gene, y=Protein_position_relative )) +
#  geom_boxplot(aes(group = Gene)) +
#  geom_point(aes(color=log(`Selective pressure`))) +
#  facet_grid(Inheritance ~ .)

p2 <- subset %>% 
  ggplot() +
  geom_point(aes(x=Protein_position_relative, 
                 y=`Selective pressure`,
                 group = Gene)) 

p2log <- subset %>% 
  ggplot() +
  geom_point(aes(x=Protein_position_relative, 
                 y= log(`Selective pressure`),
                 group = Gene)) 
